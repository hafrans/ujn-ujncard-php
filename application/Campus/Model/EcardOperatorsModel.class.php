...
class EcardOperatorsModel extends CommonModel
{

    protected $_validate = array(
        //array(验证字段,验证规则,错误提示,验证条件,附加规则,验证时间)
        array('operator', 'require', '账号不能为空', 1, 'regex', CommonModel:: MODEL_INSERT),
        array('nickname', 'require', '昵称不能为空', 1, 'regex', CommonModel:: MODEL_BOTH),
        array('password', 'require', '密码不能为空', 1, 'regex', CommonModel:: MODEL_INSERT),
        array('repassword', 'password', '两次密码不一致', 0, 'confirm', CommonModel:: MODEL_INSERT),
        
        array('operator', '', '账号已经存在！', 1, 'unique', CommonModel:: MODEL_BOTH  ),
...
//     }
    ...
    public static function get_current_userid(){
        if(isset($_SESSION['optr']['id'])){
            return $_SESSION['optr']['id'];
        }else{
            return 0;
        }
    }
